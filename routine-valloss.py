...
def train_model(batch_size, epochs, learn_rate, name, listener_state, speller_state):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optim, step_size=2, gamma=0.7)
    ...
    for epoch in range(epochs):
        ...
        for (utterance, frame_lens, Yinput, Ytarget, transcript_lens) in dev_dataloader:
            ...
            # forward
            key, value = listener(to_variable(utterance), frame_lens)
            pred_seq = speller(key, value, to_variable(Yinput), Yinput.size(-1), False, frame_lens).squeeze()  # input prev pred

            pred_seq = pred_seq[:max(transcript_lens), :]
            if pred_seq.size(0) < max(transcript_lens):
                pred_seq = torch.cat([pred_seq, to_variable(torch.ones((max(transcript_lens) - pred_seq.size(0), char_count)))], dim=0)

            gold = to_variable(Ytarget).squeeze()
            # print(pred_seq.size())
            # print(gold.size())
            # loss
            loss = loss_fn(pred_seq, gold)
            loss = torch.sum(loss) / actual_batch_size
...
train_model(batch_size=32, epochs=15, learn_rate=1e-3, name='try7',
...
try7: teacher force 0.6, lr decay
'''